...
async def add_model(model: ModelSchema, dependencies=Depends(JWTBearer()), db: Session = Depends(get_db)):
    ...
    model = await service.add_model(user_id=get_user_id_from_token(dependencies), model=model, db=db)
...
async def update_model(
    data_update: ModelSchema, model_id: int, dependencies=Depends(JWTBearer()), db: Session = Depends(get_db)
):
    ...
    if not db_brand:
        return Response(status_code=HTTP_404_NOT_FOUND, content="Brand not found")
    model = await service.update_model(user_id=get_user_id_from_token(dependencies), model=db_model, data_update=data_update, db=db)
    return model


@models.delete("/{model_id}", status_code=HTTP_200_OK)
async def delete_model(model_id: int, dependencies=Depends(JWTBearer()), db: Session = Depends(get_db)):
    model = await service.get_model(model_id, db)
    if not model:
        return Response(status_code=HTTP_404_NOT_FOUND)
    await service.delete_model(user_id=get_user_id_from_token(dependencies), model=model, db=db)
