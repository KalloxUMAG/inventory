...
async def add_equipment_type(equipment_type: EquipmentTypeSchema, dependencies=Depends(JWTBearer()), db: Session = Depends(get_db)):
    ...
    new_equipment_type = await service.add_equipment_type(user_id=get_user_id_from_token(dependencies), equipment_type=equipment_type, db=db)
...
async def add_equipment(equipment: EquipmentSchema, dependencies=Depends(JWTBearer()), db: Session = Depends(get_db)):
    if equipment.supplier_id is not None:
        db_supplier = await supplier_service.get_supplier(equipment.supplier_id, db=db)
        if not db_supplier:
            return Response(status_code=HTTP_404_NOT_FOUND, content="Supplier not found")
    if equipment.invoice_id is not None:
        ...
        if not db_invoice:
            ...
            return Response(status_code=HTTP_404_NOT_FOUND, content="Invoice not found")
    if equipment.model_number_id is not None:
        ...
        if not db_model_number:
            return Response(status_code=HTTP_404_NOT_FOUND, content="Model number not found")
    if equipment.room_id is not None:
        db_room = await room_service.get_room(equipment.room_id, db=db)
        if not db_room:
            return Response(status_code=HTTP_404_NOT_FOUND, content="Room not found")
    if equipment.stage_id is not None:
        db_stage = await get_stage(equipment.stage_id, db=db)
        if not db_stage:
            return Response(status_code=HTTP_404_NOT_FOUND, content="Stage not found")
    if equipment.equipment_type_id is not None:
        ...
        if not db_equipment_type:
            return Response(status_code=HTTP_404_NOT_FOUND, content="Equipment type not found")

    new_equipment = await service.add_equipment(user_id=get_user_id_from_token(dependencies), equipment=equipment, db=db)
...
async def add_image(equipment_id: int, file: UploadFile, dependencies=Depends(JWTBearer())):
    await service.add_image(user_id=get_user_id_from_token(dependencies), equipment_id=equipment_id, file=file)
...
async def delete_equipment(equipment_id: int, dependencies=Depends(JWTBearer()), db: Session = Depends(get_db)):
    db_equipment = await service.get_equipment_simple(equipment_id, db)
    if not db_equipment:
        return Response(status_code=HTTP_404_NOT_FOUND)
    await service.delete_equipment(user_id=get_user_id_from_token(dependencies), equipment=db_equipment, db=db)
    return Response(status_code=HTTP_204_NO_CONTENT)


@equipments.put("/{equipment_id}", status_code=HTTP_200_OK)
async def update_equipment(
    data_update: UpdateEquipmentSchema, equipment_id: int, dependencies=Depends(JWTBearer()), db: Session = Depends(get_db)
):
    db_equipment = await service.get_equipment_simple(equipment_id, db)
    if not db_equipment:
        return Response(status_code=HTTP_404_NOT_FOUND)
    update_equipment = await service.update_equipment(user_id=get_user_id_from_token(dependencies), equipment=db_equipment, data_update=data_update, db=db)
...
async def delete_image(equipment_id: int, file: UploadFile, dependencies=Depends(JWTBearer())):
    await service.delete_image(user_id=get_user_id_from_token(dependencies), equipment_id=equipment_id, file=file)
